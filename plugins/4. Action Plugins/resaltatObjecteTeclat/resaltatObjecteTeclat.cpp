...
void ResaltatObjecteTeclat::pintaBox(int i) {
...
void ResaltatObjecteTeclat::onSceneClear() {
     seleccionat = -1;	
}

void ResaltatObjecteTeclat::onObjectAdd()
{
	scene()->setSelectedObject(seleccionat+1);
	seleccionat = scene()->selectedObject();
}

void ResaltatObjecteTeclat::createBox() {
    GLWidget &g = *glwidget();
    g.makeCurrent();    
    if (!created)
    {	
...
void ResaltatObjecteTeclat::keyPressEvent(QKeyEvent *ke) {
	switch (ke->key()){
		case Qt::Key_0:
